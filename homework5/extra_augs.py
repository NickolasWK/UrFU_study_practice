...
class RandomBlur:
    """Cлучайное гауссово размытие к изображению."""
    def __init__(self, p=0.5, radius_range=(0.5, 3.0)):
        if not (0.0 <= p <= 1.0):
            raise ValueError(f"Вероятность p должна быть в диапазоне [0.0, 1.0], получено {p}")
        if not (radius_range[0] >= 0.0 and radius_range[0] <= radius_range[1]):
            raise ValueError(f"Некорректный диапазон радиусов: {radius_range}")
...
class RandomBrightness:
    """Случайным образом изменяет яркость изображения."""
    def __init__(self, p=0.5, brightness_range=(0.5, 1.5)):
        if not (0.0 <= p <= 1.0):
            raise ValueError(f"Вероятность p должна быть в диапазоне [0.0, 1.0], получено {p}")
        if not (brightness_range[0] > 0.0 and brightness_range[0] <= brightness_range[1]):
            raise ValueError(f"Некорректный диапазон яркости: {brightness_range}. Нижняя граница должна быть > 0.")
    ...
    def __call__(self, img):
        if random.random() < self.p:
            ...
            return F_tv.adjust_brightness(img, factor)
        return img


class RandomContrast:
    """Случайным образом изменяет контрастность изображения."""
    def __init__(self, p=0.5, contrast_range=(0.5, 1.5)):
        if not (0.0 <= p <= 1.0):
            raise ValueError(f"Вероятность p должна быть в диапазоне [0.0, 1.0], получено {p}")
        if not (contrast_range[0] >= 0.0 and contrast_range[0] <= contrast_range[1]):
            raise ValueError(f"Некорректный диапазон контрастности: {contrast_range}. Нижняя граница должна быть >= 0.")
    ...
    def __call__(self, img):
        if random.random() < self.p:
            ...
            return F_tv.adjust_contrast(img, factor)
        return img