...
def log_epoch(epoch, avg_loss, metrics=None, prefix=""):
    print(f"{prefix}Epoch {epoch:03d} | Loss: {avg_loss:.4f}", end="")
    if metrics:
        for name, value in metrics.items():
            if name != "ROCAUC": # ROCAUC - это объект, а не скалярная метрика для логирования здесь
